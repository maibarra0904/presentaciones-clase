...
export default function PresentationGenerator() {
  ...
  let parsedSlidesForInit: any = null
  try {
    const rawSlides = typeof localStorage !== 'undefined' ? localStorage.getItem(slidesKey) : null
    if (rawSlides) parsedSlidesForInit = JSON.parse(rawSlides)
  } catch {
    /* ignore parse errors */
  }
  ...
  async function handleGenerate() {
    ...
    setLoading(true)
    try {
      // join unit + topics into one prompt string
      const promptTopic = `${unit}${unit ? ' - ' : ''}${topics.join('; ')}`
      console.debug('[presentation] promptTopic:', promptTopic)
      console.debug('[presentation] localStorage hasKey:', Boolean(typeof localStorage !== 'undefined' && localStorage.getItem('groqApiKey')))
      const res = await generateSlides({ topic: promptTopic, slides: slidesCount, style, apiKey })
      ...
      // ensure normalized videos in returned slides
      const normalizedBody: Slide[] = Array.isArray(res)
        ? res.map(s => ({ ...s, videos: s.videos ? s.videos.map(v => toYouTubeEmbed(v)) : s.videos }))
        : []
      const finalSlides = [cover, ...normalizedBody]
      setSlides(finalSlides)
      // detect stub-like slides (heuristic)
      ...
      if (isStub) {
        setInfoMessage('Se ha usado el modo de ejemplo (stub). Verifica tu clave de API o consulta la consola para más detalles.')
        console.warn('[presentation] generateSlides returned stub-like content. Revisa la consola para errores del servicio.')
      } else {
        setInfoMessage(null)
      }
      // persist slides and form (normalize video URLs to embed form before saving)
      try {
        const wrapper = {
          metadata: { subject, teacher, logo: logoUrl, unit, topics, slidesCount, style },
          slides: finalSlides,
        }
        localStorage.setItem(slidesKey, JSON.stringify(wrapper))
      } catch (e) { console.warn('No se pudo guardar slides', e) }
      ...
      console.error(err)
      alert('Error generando las diapositivas')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium">Diapositivas</label>
  ...
  {!hasSavedKey && <button onClick={() => { if (apiKey) { localStorage.setItem('groqApiKey', apiKey); setHasSavedKey(true); alert('Clave guardada localmente') } else { alert('Introduce una clave antes de guardar') } }} className="px-4 py-2 bg-gray-100 rounded">Guardar</button>}
