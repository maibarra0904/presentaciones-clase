...
export default function PresentationPage() {
  ...
  const estimatedMaxHeight = useMemo(() => {
    ...
    const estimateFor = (s: Slide) => {
      ...
      for (const seg of segs) {
        // code segments: if python, ignore entirely; otherwise include a short preview (first 5 lines)
        if (seg.type === 'code') {
  ...
  // detect mobile to tweak spacing and apply a proportional scale
  const [isMobile, setIsMobile] = useState<boolean>(false)
  const mobileScale = 0.92 // reduce sizes by ~8% on mobile
  ...
  {/* make slide box full-width on mobile, constrained on sm+ */}
  <div ref={slideBoxRef} className="relative w-full sm:max-w-3xl sm:mx-auto px-0">
          ...
          <Tooltip title={isFirst ? 'Inicio' : 'Anterior'}>
            <Fab
              ...
              disabled={isFirst}
                sx={{
                  // fixed on mobile so buttons float relative to viewport center; absolute on md+ to stay next to the slide box
                  position: { xs: 'fixed', md: 'absolute' },
                  // on mobile keep the buttons a little inset from the screen edge; on desktop keep previous offset
                  left: { xs: 12, md: -12 },
                  top: { xs: '50%', md: '50%' },
          ...
          {/* Apply visual scale only on mobile to reduce all fonts & spacing proportionally. We expand the wrapper width so scaling doesn't clip. */}
          <div
            style={
              isMobile
